...
from .paths import BIO_WORKFLOWS_PATH
...
WORKFLOW_TEMPLATES = {
    "fastqc": {
        ...
        "trigger_keywords": ["测序质量", "fastqc", "质控", "qc", "质量检查"],
        "steps": [
            {"step": 1, "prompt": "请选择你的测序文件路径（fastq/fasta格式）", "input_type": "file", "key": "input_file"},
        ...
        "result_hint": "重点关注：Per base sequence quality、Adapter Content、Duplication Levels",
    },
    "blastn": {
        "display_name": "序列比对 (BLAST)",
        "trigger_keywords": ["比对", "blast", "序列比对", "blastn", "同源"],
        "steps": [
            {"step": 1, "prompt": "输入查询序列或选择序列文件路径", "input_type": "file_or_text", "key": "query"},
            {"step": 2, "prompt": "选择数据库（nt / nr）", "input_type": "choice", "choices": ["nt", "nr"], "key": "database"},
            {"step": 3, "prompt": "确认运行 BLAST 比对？", "input_type": "confirm"},
            {"step": 4, "prompt": "结果解读", "input_type": "result_read"},
        ],
        "tool": "blastn",
        "tool_args": '-query {query_file} -db {database} -outfmt "6 qseqid sseqid pident length evalue bitscore" -max_target_seqs 10',
        "result_hint": "关注 evalue < 1e-5、identity > 80% 的比对结果",
    },
    "primer3": {
        "display_name": "设计引物 (Primer3)",
        "trigger_keywords": ["引物", "primer", "设计引物", "pcr引物"],
        "steps": [
            {"step": 1, "prompt": "输入目标序列", "input_type": "text", "key": "sequence"},
        ...
        "tool": "primer3_core",
        "tool_args": "",
        "result_hint": "关注引物Tm值、GC含量(40-60%)、特异性",
    },
    "phylo_tree": {
        "display_name": "系统发育树",
        "trigger_keywords": ["系统发育", "进化树", "phylogenetic", "建树"],
        "steps": [
            {"step": 1, "prompt": "选择多序列比对文件", "input_type": "file", "key": "alignment_file"},
            {"step": 2, "prompt": "选择建树方法", "input_type": "choice", "choices": ["Neighbor-Joining", "Maximum-Likelihood"], "key": "method"},
            {"step": 3, "prompt": "确认运行建树？", "input_type": "confirm"},
            {"step": 4, "prompt": "结果解读", "input_type": "result_read"},
        ],
        "tool": "phylo_pipeline",
        "tool_args": "",
        "result_hint": "关注 bootstrap 值(>70较可靠)、分支长度",
    },
}
...
3. 是否有问题需要注意"""
    ...
    def advance_session(self, session_id: str, user_input: str) -> Optional[dict]:
        ...
        current_step_idx = session["step"] - 1
    ...
    def explain_result(self, session_id: str) -> Optional[str]:
        ...
        try:
            ...
            explanation = self.llm_predict(prompt, f"你是iGEM生信助手。参考要点：{result_hint}")
            return explanation
        except Exception as e:
            return f"解读失败：{e}\n\n原始输出：{session['result'][:500]}"
    ...
    def _execute_command(cmd: str, timeout: int = 120) -> str:
        ...
        try:
            ...
            return f"工具未找到，请确认 {cmd.split()[0]} 已安装"
    ...
    def _execute_primer3(self, params: dict) -> str:
        ...
        try:
            input_text = (
                ...
                f"PRIMER_MIN_TM=55\n"
                f"PRIMER_MAX_TM=65\n"
                f"PRIMER_PRODUCT_SIZE_RANGE=100,500\n"
            ...
            return "[primer3_core 未安装] 请安装: pip install primer3-py"
