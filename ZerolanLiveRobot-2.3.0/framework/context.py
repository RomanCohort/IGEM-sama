...
class ZerolanLiveRobotContext:
    ...
    def __init__(self):
        ...
        if _config.pipeline.vec_db.enable:
            ...
            # Knowledge base pipeline reuses the Milvus vector DB
            if _config.pipeline.kb.enable:
                from knowledge_base.kb_pipeline import KnowledgeBasePipeline
                self.kb_pipeline = KnowledgeBasePipeline(self.vec_db, _config.pipeline.kb)
            else:
                self.kb_pipeline = None
