...
class AsyncTaskExecutor:
    def __init__(self, max_queue_size: int = 100):
        self._async_tasks: Queue[AsyncCoro] = Queue(maxsize=max_queue_size)
    ...
    def add_async_task(self, func: AsyncCoro):
        try:
            ...
            logger.warning(f"Async task queue full, dropped: {func.name} (total dropped: {self._dropped_count})")


class SyncTaskExecutor:
    def __init__(self, max_workers: int = 8, max_queue_size: int = 100):
        self._sync_tasks: queue.Queue = queue.Queue(maxsize=max_queue_size)
    ...
    def add_sync_task(self, func: SyncFunc):
        try:
            ...
            self._dropped_count += 1
            logger.warning(f"Sync task queue full, dropped: {func.name} (total dropped: {self._dropped_count})")
...
class TypedEventEmitter:
    ...
    """
    def __init__(self, max_sync_workers: int = 8, sync_queue_size: int = 100, async_queue_size: int = 100):
        self._sync_executor = SyncTaskExecutor(max_workers=max_sync_workers, max_queue_size=sync_queue_size)
        self._async_executor = AsyncTaskExecutor(max_queue_size=async_queue_size)
    ...
    def _create_tasks(self, event: str, event_data: BaseEvent):
        with self._listeners_lock:
            ...
            if listeners:
                # Collect once listeners to remove after iteration (fixes iteration-modification bug)
                to_remove = []
